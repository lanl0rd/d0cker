...
if [[ "$TARGETPLATFORM" == *"arm"* ]] ; then command="add-apt-repository \"deb [arch=arm64] https://download.docker.com/linux/debian buster stable\"" ; fi;

echo "I am running on $BUILDPLATFORM, building for $TARGETPLATFORM " && "dpkg --print-architecture" && \
apt-get update && \
