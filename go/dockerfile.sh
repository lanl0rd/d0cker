...
if [[ "$TARGETPLATFORM" == *"arm"* ]] ; then command="add-apt-repository \"deb [arch=arm64] https://download.docker.com/linux/debian buster stable\""; dpkg --add-architecture arm64; dpkg --remove-architecture amd64 ; fi;
